...
BOT_NAME = "ربات AGN021G"
BOT_NAME_EN = "AGN021G Bot"
SUPPORT_TG = "https://t.me/AGN021G"
GROUP_TG = "https://t.me/AGN021GCHAT"
CHANNEL_TG = "https://t.me/AGN021G1388"
CHANNEL_USERNAME = "@AGN021G1388"
...
def _load_force_join():
    ...
    try:
        ...
        if p.exists():
            ...
            FORCE_JOIN["enabled"] = bool(fj.get("enabled", True))
            ch = str(fj.get("channel") or "").strip()
            FORCE_JOIN["channel"] = ch or "@AGN021G1388"
...
def _main_menu_kb(chat_id: int | None = None):
    """منوی ادمین یا کاربر"""
    if chat_id is not None and _is_admin(chat_id):
        return {
            "inline_keyboard": [
                [{"text": "🛒 فروشگاه (نمای کاربر)", "callback_data": "shop"}],
                [{"text": "📊 داشبورد و آمار", "callback_data": "stats"}],
                [
                ...
                [{"text": "📦 مدیریت محصولات", "callback_data": "adm:products"}],
                [{"text": "💳 تنظیم کارت بانکی", "callback_data": "adm:card"}],
                [{"text": "✅ سفارش‌های در انتظار", "callback_data": "adm:pending"}],
                [{"text": "⚙️ تنظیمات ربات", "callback_data": "settings"}],
                [
                    {"text": "💬 پشتیبانی", "url": SUPPORT_TG},
                    {"text": "👥 گروه", "url": GROUP_TG},
                ],
                [{"text": "📢 کانال AGN021G", "url": CHANNEL_TG}],
            ]
        }
    return {
        "inline_keyboard": [
...
def _shop_kb():
    rows = []
    for p in shop.list_products(active_only=True):
        rows.append([{
            "text": f"{p.get('name')} — {shop.fmt_price(int(p.get('price') or 0))}",
            "callback_data": f"buy:{p.get('id')}",
        }])
    if not rows:
        rows.append([{"text": "فعلاً محصولی نیست", "callback_data": "menu"}])
    rows.append([{"text": "🏠 منو", "callback_data": "menu"}])
    return {"inline_keyboard": rows}


def _product_buy_kb(pid: str):
    return {
        "inline_keyboard": [
            [{"text": "💳 پرداخت کارت‌به‌کارت", "callback_data": f"paycard:{pid}"}],
            [{"text": "💰 خرید از موجودی کیف پول", "callback_data": f"paywallet:{pid}"}],
            [{"text": "🔙 بازگشت", "callback_data": "shop"}],
        ]
    }


def _pending_order_kb(oid: str):
    return {
        "inline_keyboard": [
            [
                {"text": "✅ تأیید و ساخت کانفیگ", "callback_data": f"ord:ok:{oid}"},
                {"text": "❌ رد", "callback_data": f"ord:no:{oid}"},
            ],
...
# ── Text formatters ──────────────────────────────────────────────────────────
def _welcome_text(chat_id: int | None = None):
    if chat_id is not None and _is_admin(chat_id):
        return (
            f"✨ <b>{BOT_NAME}</b> — پنل ادمین\n"
            f"مدیریت کانفیگ، فروش و سفارش‌ها\n\n"
            f"💬 پشتیبانی: <a href=\"{SUPPORT_TG}\">@AGN021G</a>"
        )
    return (
        f"✨ <b>{BOT_NAME}</b>\n"
        f"خرید سرویس VPN امن و سریع\n\n"
        f"از منوی زیر سرویس بخرید یا موجودی کیف پول را ببینید.\n\n"
        f"💬 پشتیبانی: <a href=\"{SUPPORT_TG}\">@AGN021G</a>\n"
        f"👥 گروه: <a href=\"{GROUP_TG}\">AGN021GCHAT</a>\n"
        f"📢 کانال: <a href=\"{CHANNEL_TG}\">AGN021G1388</a>"
    )
...
async def _deliver_order(order: dict):
    """ساخت کانفیگ از روی سفارش فروش"""
    from main import make_link, parse_size_to_bytes, vless_link_for_link, get_host
    vol = float(order.get("volume_gb") or 0)
    limit_bytes = int(vol * 1024 * 1024 * 1024) if vol > 0 else 0
    days = int(order.get("days") or 0)
    expires = None
    if days > 0:
        expires = (datetime.now() + timedelta(days=days)).isoformat()
    label = f"{order.get('product_name') or 'Shop'}-{order.get('user_id')}"
    uid, link = await make_link(
        label=label[:40],
        limit_bytes=limit_bytes,
        expires_at=expires,
        protocol=order.get("protocol") or "vless-ws",
        note=f"order:{order.get('id')} user:{order.get('user_id')}",
    )
    host = ""
    try:
        host = get_host(None) or ""
    except Exception:
        pass
    import os
    host = host or (os.environ.get("RAILWAY_PUBLIC_DOMAIN") or os.environ.get("PUBLIC_HOST") or "localhost")
    sub_url = f"https://{host}/sub/{uid}"
    return uid, link, sub_url
...
async def _handle_message(msg: dict):
    ...
    if st.get("action") == "newsub_name":
        ...
        try:
            sid, sub = await create_sub_group(name=name, desc="از ربات پنل")
            _pending.pop(chat_id, None)
    ...
    if text.startswith("/start") or text in ("/menu", "منو"):
        if not await _guard(chat_id, user_id):
            return
        await _send(chat_id, _welcome_text(chat_id), _main_menu_kb(chat_id))
        return
    ...
    # shop: card receipt
    if st.get("action") == "wait_receipt":
        if not await _guard(chat_id, user_id):
            return
        oid = st.get("order_id")
        receipt = text[:500]
        if msg.get("photo"):
            receipt = f"[photo:{msg['photo'][-1].get('file_id','')}] " + receipt
        o = shop.set_order_receipt(oid, receipt or "رسید ارسال شد")
        _pending.pop(chat_id, None)
        if o:
            await _send(chat_id, "✅ رسید ثبت شد. منتظر تأیید ادمین بمانید.", _main_menu_kb(chat_id))
            for aid in ADMIN_IDS:
                try:
                    await _send(
                        aid,
                        f"📥 <b>سفارش جدید</b>\n"
                        f"کاربر: <code>{o.get('user_id')}</code> @{o.get('username') or '-'}\n"
                        f"محصول: {o.get('product_name')}\n"
                        f"مبلغ: {shop.fmt_price(int(o.get('price') or 0))}\n"
                        f"رسید: {o.get('receipt')}",
                        _pending_order_kb(oid),
                    )
                except Exception:
                    pass
        else:
            await _send(chat_id, "❌ سفارش پیدا نشد.", _main_menu_kb(chat_id))
        return

    # admin: set card number
    if st.get("action") == "set_card":
        if not _is_admin(chat_id):
            return
        parts = text.strip().split("|", 1)
        num = parts[0].strip()
        holder = parts[1].strip() if len(parts) > 1 else ""
        shop.set_card(num, holder)
        _pending.pop(chat_id, None)
        await _send(chat_id, f"✅ کارت ذخیره شد:\n<code>{num}</code>\n{holder}", _main_menu_kb(chat_id))
        return

    # admin: add product wizard text
    if st.get("action") == "add_product":
        if not _is_admin(chat_id):
            return
        # format: name|price|gb|days|protocol
        parts = [x.strip() for x in text.split("|")]
        if len(parts) < 4:
            await _send(chat_id, "فرمت: نام|قیمت|حجم‌GB|روز|پروتکل\nمثال: یک‌ماهه|150000|50|30|vless-ws")
            return
        try:
            name, price, gb, days = parts[0], int(parts[1]), float(parts[2]), int(parts[3])
            proto = parts[4] if len(parts) > 4 else "vless-ws"
            prod = shop.add_product(name, price, gb, days, proto)
            _pending.pop(chat_id, None)
            await _send(chat_id, f"✅ محصول اضافه شد: {prod['name']} ({prod['id']})", _main_menu_kb(chat_id))
        except Exception as e:
...
async def _handle_callback(cb: dict):
    ...
    if data == "shop":
        ...
        if not await _guard(chat_id, user_id):
            return
        await _edit(chat_id, mid, "🛍 <b>فروشگاه AGN021G</b>\nیک محصول را انتخاب کنید:", _shop_kb())
        return
    ...
    if data == "wallet":
        ...
        if not await _guard(chat_id, user_id):
            return
        w = shop.get_wallet(user_id)
        await _edit(
            chat_id, mid,
            f"💼 <b>کیف پول</b>\nموجودی: <b>{shop.fmt_price(int(w.get('balance') or 0))}</b>\n\n"
            f"برای افزایش موجودی از پشتیبانی کمک بگیرید یا بعد از خرید کارت‌به‌کارت تأیید شود.",
            _main_menu_kb(chat_id),
        )
        return
    ...
    if data == "myorders":
        ...
        if not await _guard(chat_id, user_id):
            return
        orders = shop.list_user_orders(user_id)[:10]
        if not orders:
            await _edit(chat_id, mid, "📦 سفارشی ندارید.", _main_menu_kb(chat_id))
            return
        lines = []
        for o in orders:
            lines.append(
                f"• <b>{o.get('product_name')}</b> — {shop.fmt_price(int(o.get('price') or 0))}\n"
                f"  وضعیت: <code>{o.get('status')}</code>"
            )
        await _edit(chat_id, mid, "📦 <b>سفارش‌های شما</b>\n\n" + "\n".join(lines), _main_menu_kb(chat_id))
        return
    ...
    if data.startswith("buy:"):
        ...
        if not prod:
            await _edit(chat_id, mid, "محصول یافت نشد.", _shop_kb())
            return
        vol = prod.get("volume_gb") or 0
        days = prod.get("days") or 0
        txt = (
            f"📦 <b>{prod.get('name')}</b>\n"
            f"💰 قیمت: {shop.fmt_price(int(prod.get('price') or 0))}\n"
            f"📊 حجم: {vol if vol else '∞'} GB\n"
            f"⏱ مدت: {days if days else '∞'} روز\n"
            f"🔌 پروتکل: {prod.get('protocol')}\n\n"
            f"روش پرداخت را انتخاب کنید:"
        )
        await _edit(chat_id, mid, txt, _product_buy_kb(pid))
        return
    ...
    if data.startswith("paycard:"):
        ...
        if not prod:
            await _edit(chat_id, mid, "محصول یافت نشد.", _shop_kb())
            return
        card, holder = shop.get_card()
        if not card:
            await _edit(chat_id, mid, "⚠️ کارت بانکی هنوز تنظیم نشده. به پشتیبانی پیام دهید.", _main_menu_kb(chat_id))
            return
        user = cb.get("from") or {}
        ...
        if not order:
            await _edit(chat_id, mid, "خطا در ساخت سفارش.", _shop_kb())
            return
        _pending[chat_id] = {"action": "wait_receipt", "order_id": order["id"]}
        await _edit(
            chat_id, mid,
            f"💳 <b>پرداخت کارت‌به‌کارت</b>\n"
            f"مبلغ: <b>{shop.fmt_price(int(order['price']))}</b>\n"
            f"کارت: <code>{card}</code>\n"
            f"{('به نام: ' + holder) if holder else ''}\n\n"
            f"پس از واریز، <b>رسید یا کد پیگیری</b> را همین‌جا ارسال کنید.",
            {"inline_keyboard": [[{"text": "❌ انصراف", "callback_data": "shop"}]]},
        )
        return
    ...
    if data.startswith("paywallet:"):
        ...
        if not shop.deduct_balance(user_id, price):
            await _edit(chat_id, mid, "موجودی کافی نیست. از کارت‌به‌کارت استفاده کنید.", _product_buy_kb(pid))
            return
        user = cb.get("from") or {}
        order = shop.create_order(user_id, pid, username=str(user.get("username") or ""), full_name=str(user.get("first_name") or ""))
        if not order:
            shop.add_balance(user_id, price)
            await _edit(chat_id, mid, "خطا — موجودی برگشت داده شد.", _shop_kb())
            return
        # auto deliver
        uuid, link, sub_url = await _deliver_order(order)
        shop.approve_order(order["id"], uuid)
        await _edit(
            chat_id, mid,
            f"✅ خرید موفق!\n{order['product_name']}\n\n"
            f"🔗 ساب:\n<code>{sub_url}</code>",
    ...
    if data == "adm:products":
        ...
        if not _is_admin(chat_id):
            return
        prods = shop.list_products(active_only=False)
        lines = ["📦 <b>محصولات</b>\n"]
        rows = []
        for p in prods:
            st = "🟢" if p.get("active", True) else "🔴"
            lines.append(f"{st} {p.get('name')} — {shop.fmt_price(int(p.get('price') or 0))} (<code>{p.get('id')}</code>)")
            rows.append([{"text": f"🗑 حذف {p.get('name')[:20]}", "callback_data": f"adm:delp:{p.get('id')}"}])
        rows.append([{"text": "➕ افزودن محصول", "callback_data": "adm:addp"}])
        rows.append([{"text": "🏠 منو", "callback_data": "menu"}])
        await _edit(chat_id, mid, "\n".join(lines) if len(lines) > 1 else "محصولی نیست.", {"inline_keyboard": rows})
        return
    ...
    if data == "adm:addp":
        ...
        if not _is_admin(chat_id):
            return
        _pending[chat_id] = {"action": "add_product"}
        await _edit(
            chat_id, mid,
            "فرمت محصول را بفرستید:\n<code>نام|قیمت|حجم‌GB|روز|پروتکل</code>\n"
            "مثال:\n<code>یک‌ماهه ۵۰گیگ|150000|50|30|vless-ws</code>",
    ...
    if data.startswith("adm:delp:"):
        ...
        await _edit(chat_id, mid, "حذف شد.", _main_menu_kb(chat_id))
        return
    ...
    if data == "adm:card":
        ...
        if not _is_admin(chat_id):
            return
        card, holder = shop.get_card()
        _pending[chat_id] = {"action": "set_card"}
        await _edit(
            chat_id, mid,
            f"کارت فعلی: <code>{card or '—'}</code>\n{holder or ''}\n\n"
            f"شماره کارت جدید را بفرستید:\n<code>6037...|نام صاحب کارت</code>",
            {"inline_keyboard": [[{"text": "❌ انصراف", "callback_data": "menu"}]]},
        )
        return
    ...
    if data == "adm:pending":
        ...
        if not pending:
            await _edit(chat_id, mid, "سفارش در انتظاری نیست.", _main_menu_kb(chat_id))
            return
        o = pending[0]
        await _edit(
            chat_id, mid,
            f"📥 سفارش <code>{o.get('id')}</code>\n"
            f"کاربر: {o.get('user_id')} @{o.get('username') or '-'}\n"
            f"{o.get('product_name')} — {shop.fmt_price(int(o.get('price') or 0))}\n"
            f"رسید: {o.get('receipt')}",
            _pending_order_kb(o["id"]),
        )
        return
    ...
    if data.startswith("ord:ok:"):
        ...
        if not _is_admin(chat_id):
            return
        oid = data.split(":")[-1]
        order = shop.get_order(oid)
        if not order or order.get("status") != "pending_review":
            await _edit(chat_id, mid, "این سفارش قابل تأیید نیست.", _main_menu_kb(chat_id))
            return
        uuid, link, sub_url = await _deliver_order(order)
        shop.approve_order(oid, uuid)
        # credit optional: nothing
        await _edit(chat_id, mid, f"✅ تأیید شد و کانفیگ ساخته شد.\n{sub_url}", _main_menu_kb(chat_id))
        try:
            await _send(
                int(order["user_id"]),
                f"✅ پرداخت شما تأیید شد!\n{order.get('product_name')}\n\n🔗 ساب:\n<code>{sub_url}</code>",
                _main_menu_kb(int(order["user_id"])),
            )
        except Exception:
            pass
        return

    if data.startswith("ord:no:"):
        await _answer_cb(cb_id)
        if not _is_admin(chat_id):
            return
        oid = data.split(":")[-1]
        shop.reject_order(oid, "توسط ادمین")
        order = shop.get_order(oid)
        await _edit(chat_id, mid, "سفارش رد شد.", _main_menu_kb(chat_id))
        if order:
            try:
                await _send(int(order["user_id"]), "❌ پرداخت شما رد شد. با پشتیبانی تماس بگیرید.", _main_menu_kb(int(order["user_id"])))
            except Exception:
                pass
        return
...
async def start_bot(mode: str = "polling"):
    ...
    try:
        ...
        await _call(
            "setMyDescription",
            description=f"{BOT_NAME} ({BOT_NAME_EN}) — مدیریت کامل پنل از تلگرام",
        )
        await _call(
            "setMyCommands",
            commands=[
